...
EXTEND_BOTTOM BDLIIA 14
	IF ~OR(2) CheckStatGT(Player1,15,WIS) CheckStatGT(Player1,15,INT)~ THEN REPLY @100	/* ~They used a weak poison?  That makes no...they weren't trying to kill me.  They were trying to capture me.~ */ DO ~SetGlobal("#L_WeakPoison","GLOBAL",1)~ GOTO LIIA_WEAK_POISON
END
...
END
EXTEND_BOTTOM BDDYNAHE 33
	IF ~Global("#L_WeakPoison","GLOBAL",1)~ THEN REPLY @120 /* ~She attacked me and Imoen, and she will pay for that mistake. That is all you need to know.~ */ GOTO 35
END
...
EXTEND_BOTTOM BDRASAAD 38
	IF ~Global("#L_WeakPoison","GLOBAL",1)~ THEN REPLY #234637 /* ~The Shining Lady Caelar happened. Or assassins who bore her mark did, at any rate.~ */ GOTO RASAAD_ATTACK
	IF ~Global("#L_WeakPoison","GLOBAL",1)~ THEN REPLY @128 /* ~Assassins in service to Caelar Argent penetrated the Ducal Palace and attempted to poison me.~ */ GOTO 25
END
...
EXTEND_BOTTOM BDRASAAD 39
	IF ~Global("#L_WeakPoison","GLOBAL",1)~ THEN REPLY @129 /* ~Like them, my life has suffered from her malign influence. Her assassins tried to poison me earlier. They nearly succeeded.~ */ GOTO RASAAD_ATTACK
	IF ~Global("#L_WeakPoison","GLOBAL",1)~ THEN REPLY @130 /* ~Has she sent multiple assassins to poison any of them? Because that's what she did to me.~ */ GOTO RASAAD_ATTACK
	IF ~Global("#L_WeakPoison","GLOBAL",1)~ THEN REPLY @131 /* ~These people have run from their troubles. I intend to face mine. Caelar tried to poison me this morning.~ */ GOTO RASAAD_ATTACK
END
